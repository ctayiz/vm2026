import { db } from "./db";
import { fetchTopScorers, fetchFixtures, fetchFixtureGoals, playerPhotoUrl, hasApiFootball } from "./api-football";
...
export async function syncTopScorers(): Promise<ScorerSyncSummary> {
  ...
  try {
    ...
    // Fotos für Spieler ergänzen, die noch keins haben (z. B. aus Match-Events angelegt).
    // URL-Muster: media.api-sports.io/football/players/{externalId}.png – kein API-Call nötig.
    // Nur numerische externalIds (API-Football) – keine alten "fd-"-IDs.
    const noPhoto = await db.player.findMany({
      where: { photo: null, goals: { gt: 0 } },
      select: { id: true, externalId: true },
    });
    for (const p of noPhoto) {
      if (!p.externalId || !/^\d+$/.test(p.externalId)) continue;
      await db.player.update({ where: { id: p.id }, data: { photo: playerPhotoUrl(p.externalId) } });
    }
